...
def combine2Pdf(folderPath, pdfFilePath):
    ...
    output.save(pdfFilePath + '/result.pdf', "pdf", save_all=True, append_images=sources)
...
def pyMuPDF_fitz(pdfPath, imagePath):
    ...
    for pg in range(pdfDoc.pageCount):
        ...
        if not os.path.exists(imagePath + '/' + 'images_%s' % filename):  # 判断存放图片的文件夹是否存在
            os.makedirs(imagePath + '/' + 'images_%s' % filename)  # 若图片文件夹不存在就创建
        print(imagePath + '/' + 'images_%s/%s.png' % (filename, i))
        pix.writePNG(imagePath + '/' + 'images_%s/%s.png' % (filename, i))  # 将图片写入指定的文件夹内
